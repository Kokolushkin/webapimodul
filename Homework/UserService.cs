    ...
    public class UserService : IUserService
    {
        ...
        public bool Transfer(User userFrom, User userTo, double howmuch)
        {
            if(userFrom.Balance >= howmuch)
            {
                using (var conn = CreateConnection())
                {
                    ...
                    conn.Execute("UPDATE users1 SET balance = @balance WHERE accountNumber = @AccountNumber;",
                        new
                        {
        ...
        public User GetUser(double accountnumber)
        {
            using (var conn = CreateConnection())
            {
                conn.Open();
                    ...
                    var result = conn.QuerySingleOrDefault<User>("SELECT accountNumber, balance FROM users1 WHERE accountNumber = @accountnumber;",
                    new
                    {
                        accountnumber,
                    });
        ...
        public void CreateUser(UserCredentials user)
        {
            using (var conn = CreateConnection())
            {
                ...
                conn.Execute("INSERT INTO users1 (username, email, passwordHash, salt, accountNumber, balance) VALUES(@username, @email, @passwordHash, @salt, @accountNumber, @balance);",
                    new
                    {
        ...
        public bool CheckUniqueNameAndEmail(String username, String email)
        {
            using (var conn = CreateConnection())
            {
                ...
                var checkUniqueName = conn.QuerySingleOrDefault<User>("SELECT username FROM users1 WHERE username = @Username;",
                    new
                    {
                        username,
                    });

                var checkUniqueEmail = conn.QuerySingleOrDefault<User>("SELECT email FROM users1 WHERE email = @Email;",
                    new
                    {
                        email,
                    });
        ...
        private bool CheckUniqueAccountNumber(long number)
        {
            using (var conn = CreateConnection())
            {
                ...
                var checkUniqueAccountNumber = conn.QuerySingleOrDefault<User>("SELECT accountNumber FROM users1 WHERE accountNumber = @number;",
                    new
                    {
                        number,
                    });
        ...
        {
            var connection = new NpgsqlConnection($"server=localhost;database=Example1;userid=postgres;password=12345;Pooling=false");
